...
let page = 1,
  links = [],
  heartbeat = null,
  bar = new progress(':bar :percent :len link saved', {
    total: config.limit ? config.limit : config.end - config.start,
    width: 20,
  }),
  special = typeof config.special === 'string'
    ? config.special
        .split(',')
        .map(d => ({
          [d.split(':')[0].trim()]: d.split(':')[1].trim().split('*'),
        }))
        .reduce((a, b) => Object.assign(a, b))
    : null;
try {
  if (config.heartbeat) heartbeat = require(`${process.cwd()}/heartbeat.js`);
} catch (e) {
...
const queue = new PQueue({retry: true}),
  crawl = body => {
    ...
    if (
      $(config.list).length &&
      page <= (config.limit ? config.limit : config.end - config.start)
    ) {
      bar.tick({
        len: links.length,
      });
      ...
      $(config.list).each((i, el) => {
          ...
          if (special[elem][0].match(/[<\^>]/)) {
            ...
            if (attr === 'text') {
              ...
              if (attr != 'src') {
                ...
                item[key] = url.resolve(
                  main,
                  elem.find(selector).attr(attr)
                );
              }
            }
          } else {
            if (special[elem][1] === 'text') {
              ...
              if (special[elem][1] != 'src') {
                item[elem] = $(el)
                  .find(special[elem][0])
                  .attr(special[elem][1]);
              } else {
                item[elem] = url.resolve(
                  main,
                  $(el)
                    .find(special[elem][0])
                    .attr(special[elem][1])
        ...
        if (is.filter(e => e).length > (config.min || 1)) links.push(item);

        if (heartbeat) heartbeat(item);
      });
      ...
      console.log(
        `Completed in ${ms(+new Date() - start)}.\n${links.length} link saved to ${config.file || 'report'}.json file.`
      );

      json.writeFileSync(
        `${process.cwd()}/${config.file || 'report'}.json`,
        links
      );
    }
  };
